...
GDP_DJIA |>
  summarise(
    correlation = cor(GDP, DJIA, method="pearson")
  )
```
...
GDP_DJIA |> 
  ...
  labs(
    title = "The Gross Domestic Product and Dow Jones Industrial Average Correlation 2013-2024",
    x = "GDP",
    y = "DJIA"
  )
```
...
The DJIA and GDP are strongly correlated between 2013 and 2024, with a coefficient of 0.95. The scatterplot shows this correlation as well with the points being close together in the beginng. Despite the points growing slightly furhter apart later on, the data still shows a correlation. This means that the DJIA and the GDP rise and fall together, but they may not cause one another to move. 
