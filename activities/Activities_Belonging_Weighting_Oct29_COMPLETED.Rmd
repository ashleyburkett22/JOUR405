...
Statements that raise methodological concerns include:

"First-Generation status was selected for Undergraduates" --> Why was this filter put in place? It may not accurately represent the whole undergraduate class by excluding those who are not first generation. It also only includes 20% of the population. ALSO "International student status was used for Graduates" --> Another filter that I'm unsure why it was put in place. If this only represents a small amount of a whole population, why not pull from the whole population? 

"Response choices for gender identity included genderfluid, man, non-binary, woman, and a gender not listed" --> The gender category puts separate groups together into once category, which would not correctly account for different genders.

"Racial/Ethnic identity was recoded into collapsed categories for analysis purposes." --> The data was recoded for statistical significant purposes, which can mess up the results of the data. 

From my findings, it seems that the belonging survey does not accurately represent the entire campus population that it claims to show. The filters that were put on the undergraduate and graduate classes do not make much sense for me. The gender survey did not separate all identifications, and grouped some together, which would make certain populations larger/smaller than accurate. The racial/ethnic identity was also recoded for purposes of statistcial significant, which poses questions of whether the results are accurate or not. 
...
1. What is the range of weights in this dataset? **The range of weights in the dataset is from 0.2298 to 3.0386.**

2. What does it mean when a respondent has a weight of 0.23? What about 3.04? **When a respondent has a weight of 0.23, it means that they are underrepresented, and count as only 0.23 of a person. When a respondent has a weight of 3.04, it means that they are overrepresented, and count as 3.04 people, as opposed to just 1.**

3. Why might some respondents be weighted more heavily than others? **Some respondents may be more heavily weighted because they are members of minority communities, or members of a group with a low response rate. To accurately represent a whole population, these groups need to be weighted upwards to increase their affect on the results. Without the weight, their response would not have as much of an affect on the data, and the results wouldn't accurately represent a whole population. **
...
1. Which voter category shows the biggest difference between weighted and unweighted results? **The voter category with the largest difference between weighted and unweighted results is "rarely/never: yes" This category had a -3.56 percent point difference. The second most is 1.99 points, so "rarely/never: yes" surpassed the others by several percentage points.**

2. How might these differences affect reporting on likely voter turnout? **These differences may affect reporting on likely voter turnout because there is a very large difference in who rarely/never votes in a given population, from the actual respondent population to the weighted respondent population. If someone reports on the unweighted data, it would be significantly inaccurate from an accurate representation of the whole population.**

3. Why do you think these differences exist? What does this tell us about who might be over or under-represented in the sample? **I think these differences exist because there are certain groups who do and do not answer surveys. It makes sense that those who rarely/never vote would also not be likely to answer a survey. This population would need significant weighting in order ot be properly represented in a whole set of data. The percent differences tell us that those who answer surveys are going to be over represented, but those who don't answer surveys are going to be under represented.**
