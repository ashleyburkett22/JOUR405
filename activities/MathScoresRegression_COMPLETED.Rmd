...
**The relationship between FARMS percentage and the math test scores show that the higher met or exceeded math scores, the higher the FARMS percentage will be. This is shown by the regression line increasing towards the right. This is a weak positive correlation since the dots are far apart and the line is not very steep, but there is a definite slight relationship.**
...
**This model is worth using. The model tells us that there's a significant relationship between FARMS percentage and math scores. The P-value is 0.1225, and is marked by three stars. This means that there's a weak significance. The Adjusted R-squared value of 0.0254  (about 2.5%) shows that the there's an extremely small percent of scores that can be explained by the effect of the FARMS percentage. Neither the P value or adjusted r-squared values are very significant, so there's no need to look at the coefficients.**
...
**The residual shows us how far off the prediction is from the actual value. The schools with large residual scores, negative and positive, have very different predicted score than what the actual score was. For example, Whitemarsh Elementary School was -45.83 points below what the actual score was. The same goes for Crooked Billet Elementary School, where it was predicted 30.18 points above what the actual score was. These two schools have the largest differences between the predicted result and the actual math score.**
...
**The conclusion that I would draw from the model's results is that there is not a significant enough trend to support the claim that reduced price meals correlate to standardized test scores. The p-value is not significant, and the Adjusted R-squared value also does not show any significance. The chart at the beginning shows a slight weak positive correlation, but it does not prove a strong significance between the two variables as a result of the significance tests. The results would contradict the claim that the school board member had, which says that schools with fewer students eating free/reduced price meals tend to have higher test scores.**
